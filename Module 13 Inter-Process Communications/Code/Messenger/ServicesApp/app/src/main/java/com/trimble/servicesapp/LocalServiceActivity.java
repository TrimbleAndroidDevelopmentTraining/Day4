...
public class LocalServiceActivity extends Activity {
    ...
    class IncomingHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case MSG_SAY_BYE:
                    Toast.makeText(getApplicationContext(), "see ya!", Toast.LENGTH_SHORT).show();
                    break;
                default:
    ...
    protected void onResume() {
        ...
        intent.putExtra(SimpleStartedBoundService.EXTRA_MESSENGER, inMessenger);
    ...
    public void boundService_Button_onClick(View view) {
        if (outMessenger != null) {
            Message msg = Message.obtain(null, SimpleStartedBoundService.MSG_SAY_HELLO, 0, 0);
            try {
                outMessenger.send(msg);
            } catch (RemoteException e) {
                ((TextView)findViewById(R.id.time_text_view)).setText("MessengerService Error!");
            }
        }
    }
}
